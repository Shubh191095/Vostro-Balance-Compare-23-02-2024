...
public class HABIB_HABIB_BANK_LTD {
	public void testBase010(WebDriver driver) throws InterruptedException, IOException
	{
	    ...
	    //6: Accept the Pop_Up	  
		   //Identify the pop-up element using XPath (adjust the XPath as needed)
		     ...
		     okButton.click();
			    ...
			    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (HABIB)HABIB_BANK_LTD.jpg"));
...
//==========================================================================================================================================================         
	       ...
	       pw.println("Vostro Balance HABIB-PKR: " + vostroBalance);
	       pw.println("Threshold Balance For HABIB-PKR: "+properties.getProperty("HABIB-PKR"));

	       if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("HABIB-PKR"))) {

	       pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR HABIB-PKR CURRENCY");
	       }else if(Double.valueOf(properties.getProperty("HABIB-PKR")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("HABIB_Near_To_Threshold_Balance"))) {
	       pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR HABIB-PKR CURRENCY");
	       }else {
	       pw.println("BALANCE IS SUFFICIENT FOR HABIB-PKR CURRENCY");
	       }
	      // Thread.sleep(2000);
	       pw.println("*****Test Case 010 Passed For HABIB_HABIB_BANK_LTD*****");
	       pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	       pw.close();
	       }}
...
public class BM_PT_BANK_MANDIRI_PERSERO_TBK {
	public void testBase027(WebDriver driver) throws InterruptedException, IOException
	{
...
//===================================================================================================================================================     
		   ...
		   //Take Screen Shots for Select the Partner
			    ...
			    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (BM)PT_BANK_MANDIRI_PERSERO_TBK.jpg"));
...
//==========================================================================================================================================================         
	       ...
	       pw.println("Vostro Balance BM-IDR: " + vostroBalance);
			  
		   pw.println("Threshold Balance For BM-IDR: "+properties.getProperty("BM-IDR"));

	       if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("BM-IDR"))) {
              pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR BM-IDR CURRENCY");
	       }else if(Double.valueOf(properties.getProperty("BM-IDR")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("BM_Near_To_Threshold_Balance"))) {
	    	  pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR BM-IDR CURRENCY");
	       }else {
	    	  pw.println("BALANCE IS SUFFICIENT FOR BM-IDR CURRENCY");
	       }
	      // Thread.sleep(2000);
	       pw.println("*****Test Case 027 Passed For BM_PT_BANK_MANDIRI_PERSERO_TBK*****");
	       pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	       pw.close();
	       }}
...
public class SBI_State_Bank_Of_India {
	public void testBase024(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[39]")).click();
			Thread.sleep(2000);
			   ...
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
			       ...
			       //Take Screen Shots for Select the Partner
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (SBI)State_Bank_Of_India.jpg"));
...
//==========================================================================================================================================================         
	         ...
	         pw.println("Vostro Balance SBI-INR: " + vostroBalance);
			 pw.println("Threshold Balance For SBI-INR: "+properties.getProperty("SBI-INR"));

	         if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("SBI-INR"))) {

	        	 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR SBI-INR CURRENCY");
	         }else if(Double.valueOf(properties.getProperty("SBI-INR")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("SBI_Near_To_Threshold_Balance"))) {
	        	 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR SBI-INR CURRENCY");
	         }else {
	        	 pw.println("BALANCE IS SUFFICIENT FOR SBI-INR CURRENCY");
	         }
	         //Thread.sleep(2000);
	         pw.println("*****Test Case 024 Passed For SBI_State_Bank_Of_India*****");
	         pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	         pw.close();
	         }}
...
public class SAMPATH_SAMPATH_BANK_PLC {
	public void testBase021(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[36]")).click();
			Thread.sleep(2000);
...
//===================================================================================================================================================          
			   ...
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (SAMPATH)SAMPATH_BANK_PLC.jpg"));
...
//==========================================================================================================================================================         
	         ...
	         WebElement elementLKR = driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/table/tbody/tr[1]/td[2]"));
	         ...
	         String vostroBalanceLKR=elementLKR.getText().replace(",", "");
	         String vostroBalanceUSD=elementUSD.getText().replace(",", "");

	         // Print or use the retrieved text
	         pw.println("Vostro Balance LKR: " + vostroBalanceLKR);
	         pw.println("Vostro Balance USD: " + vostroBalanceUSD);

	         pw.println("Threshold Balance For SAMPATH-LKR: "+properties.getProperty("SAMPATH-LKR"));
	         pw.println("Threshold Balance For SAMPATH-USD: "+properties.getProperty("SAMPATH-USD"));
	  
	        //For PHP Currency
	 		if(Double.valueOf(vostroBalanceLKR)  < Double.valueOf(properties.getProperty("SAMPATH-LKR"))) {

	 			 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR SAMPATH-LKR CURRENCY");
	 		}else if(Double.valueOf(properties.getProperty("SAMPATH-LKR")) <  Double.valueOf(vostroBalanceLKR) && Double.valueOf(vostroBalanceLKR)  < Double.valueOf(properties.getProperty("SAMPATH_near_To_Threshold_Balance"))) {
	 			 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR SAMPATH-LKR CURRENCY");
	 		}else {
	 			 pw.println("BALANCE IS SUFFICIENT FOR SAMPATH-LKR CURRENCY");
	 		}
	 		//Thread.sleep(2000);
	 		
	        //For USD Currency		
	 		if(Double.valueOf(vostroBalanceUSD)  < Double.valueOf(properties.getProperty("SAMPATH-USD"))) {

	 			 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR SAMPATH-USD CURRENCY");
	 		}else if(Double.valueOf(properties.getProperty("SAMPATH-USD")) <  Double.valueOf(vostroBalanceUSD) && Double.valueOf(vostroBalanceUSD)  < Double.valueOf(properties.getProperty("SAMPATH_near_To_Threshold_Balance"))) {
	 			 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR SAMPATH-USD CURRENCY");
	 		}else {
	 			 pw.println("BALANCE IS SUFFICIENT FOR SAMPATH-USD CURRENCY");
	 		}
	 		//Thread.sleep(2000);
	 		  pw.println("*****Test Case 021 Passed For SAMPATH_SAMPATH_BANK_PLC*****");
	 		  pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	 		  pw.close();
	 		}}
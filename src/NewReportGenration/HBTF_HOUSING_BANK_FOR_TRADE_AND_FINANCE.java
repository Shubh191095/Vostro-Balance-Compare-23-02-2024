...
public class HBTF_HOUSING_BANK_FOR_TRADE_AND_FINANCE {
	public void testBase012(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[23]")).click();
			Thread.sleep(2000);
...
//===================================================================================================================================================     
	      //6: Accept the Pop_Up	  
		   ...
		   //Take Screen Shots for Select the Partner
			    ...
			    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (HBTF)HOUSING_BANK_FOR_TRADE_AND_FINANCE.jpg"));
...
//==========================================================================================================================================================         
	       //Get text and compare with properties file
	         ...
	         pw.println("Vostro Balance HBTF-JOD: " + vostroBalance);
			  
			 pw.println("Threshold Balance For HBTF-JOD: "+properties.getProperty("HBTF-JOD"));
	         	         
	         if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("HBTF-JOD"))) {
	          pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR HBTF-JOD CURRENCY");
	         }else if(Double.valueOf(properties.getProperty("HBTF-JOD")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("HBTF_Near_To_Threshold_Balance"))) {
	         pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR HBTF-JOD CURRENCY");
	         }else {
	         pw.println("BALANCE IS SUFFICIENT FOR HBTF-JOD CURRENCY");
	         }
	         //Thread.sleep(2000);
	         pw.println("*****Test Case 012 Passed For HBTF_HOUSING_BANK_FOR_TRADE_AND_FINANCE*****");
	         pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	         pw.close();
	         }}
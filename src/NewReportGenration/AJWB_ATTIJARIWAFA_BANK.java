...
public class AJWB_ATTIJARIWAFA_BANK {
	public void testBase003(WebDriver driver) throws InterruptedException, IOException
	{
			 ...
			 //Identify the pop-up element using XPath (adjust the XPath as needed)
			   ...
			   WebElement popup = driver.findElement(By.xpath("/html/body/div[3]/md-dialog/md-dialog-actions/button/span"));
			     ...
			     //Take Screen Shots for Select the Partner
				  ...
				  Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For AJWB(ATTIJARIWAFA_BANK).jpg"));
...
//==========================================================================================================================================================         
	...
	String vostroBalance=elementText.replace(",", "");
	 ...
	 pw.println("Vostro Balance AJWB-MAD: " + vostroBalance);  
	 pw.println("Threshold Balance For AJWB-MAD: "+properties.getProperty("AJWB-MAD"));

	if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("AJWB-MAD"))) {
		pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR AJWB-MAD currency");
		...
		pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR AJWB-MAD currency");
	}else {
		pw.println("BALANCE IS SUFFICIENT FOR AJWB-MAD currency");
	}
	//Thread.sleep(2000);
	  pw.println("*****Test Case 003 Passed For AJWB_ATTIJARIWAFA_BANK*****");
	  pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	  pw.close();
	}}
	//=====================================================================================================================================	      

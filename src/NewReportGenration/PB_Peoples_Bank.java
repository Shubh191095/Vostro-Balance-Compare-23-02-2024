...
public class PB_Peoples_Bank {
	public void testBase019(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[31]")).click();
			Thread.sleep(2000);
			   ...
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
			       ...
			       //Take Screen Shots for Select the Partner
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (PB)Peoples_Bank.jpg"));
...
//==========================================================================================================================================================         
	         ...
	         pw.println("Vostro Balance LKR: " + vostroBalance);
	         pw.println("Threshold Balance For PB-LKR: "+properties.getProperty("PB-LKR"));

	         if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("PB-LKR"))) {

	             pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR PB-LKR CURRENCY");
	         }else if(Double.valueOf(properties.getProperty("AJWB-MAD")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("PB-Near_To_Threshold_Balance"))) {
	        	 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR PB-LKR CURRENCY");
	         }else {
	        	 pw.println("BALANCE IS SUFFICIENT FOR PB-LKR CURRENCY");
	         }
	        // Thread.sleep(2000);
	         pw.println("*****Test Case 019 Passed For PB_Peoples_Bank*****");
	         pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	         pw.close();
	         }}
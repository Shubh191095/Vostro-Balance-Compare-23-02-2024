...
public class GIBL_Global_IME_Bank_Ltd {
	public void testBase009(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[19]")).click();
			Thread.sleep(2000);
	        ...
	        //6: Accept the Pop_Up	  
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (GIBL)Global_IME_Bank_Ltd.jpg"));
...
//==========================================================================================================================================================         
	       //Get text and compare with properties file
	         ...
	         pw.println("Vostro Balance NPR: " + vostroBalance);
	         pw.println("Threshold Balance For GIBL-NPR: "+properties.getProperty("GIBL-NPR"));

	         if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("GIBL-NPR"))) {
	        	 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR GIBL-NPR CURRENCY");
	        	 ...
	        	 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR GIBL-NPR CURRENCY");
	         }else {
	        	 pw.println("BALANCE IS SUFFICIENT FOR GIBL-NPR CURRENCY");
	         }
	        // Thread.sleep(2000);
	         pw.println("*****Test Case 009 Passed For GIBL_Global_IME_Bank_Ltd*****");
	         pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	         pw.close();
	         }}
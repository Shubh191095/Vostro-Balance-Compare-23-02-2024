...
public class MCB_Muslim_Commercial_Bank_Ltd {
	public void testBase017(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[28]")).click();
			Thread.sleep(2000);
			   ...
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
			       ...
			       //Take Screen Shots for Select the Partner
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (MCB)Muslim_Commercial_Bank_Ltd.jpg"));
...
//==========================================================================================================================================================         
	         ...
	         pw.println("Vostro Balance MCB-PKR: " + vostroBalance);			  
	         pw.println("Threshold Balance For MCB-PKR: "+properties.getProperty("MCB-PKR"));

	         if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("MCB-PKR"))) {

	        	 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR MCB-PKR CURRENCY");
	         }else if(Double.valueOf(properties.getProperty("MCB-PKR")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("MCB_Near_To_Threshold_Balance"))) {
	        	 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR MCB-PKR CURRENCY");
	         }else {
	        	 pw.println("BALANCE IS SUFFICIENT FOR MCB-PKR CURRENCY");
	         }
	         //Thread.sleep(2000);
	         pw.println("*****Test Case 017 Passed For MCB_Muslim_Commercial_Bank_Ltd*****");
	         pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	         pw.close();
	         }}
...
public class SEBL_SOUTHEAST_BANK_LTD {
	public void testBase023(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[38]")).click();
			Thread.sleep(2000);
			   ...
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
			       ...
			       //Take Screen Shots for Select the Partner
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For (SEBL)SOUTHEAST_BANK_LTD.jpg"));
...
//==========================================================================================================================================================         
	         ...
	         pw.println("Vostro Balance BDT: " + vostroBalance);
	         pw.println("Threshold Balance For SEBL-BDT: "+properties.getProperty("SEBL-BDT"));

	         if(Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("SEBL-BDT"))) {

	        	 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR SEBL-BDT CURRENCY");
	         }else if(Double.valueOf(properties.getProperty("SEBL-BDT")) <  Double.valueOf(vostroBalance) && Double.valueOf(vostroBalance)  < Double.valueOf(properties.getProperty("SEBL-Near_To_Threshold_Balance"))) {
	        	 pw.println("REACHING NEAR TO THRESHOLD BALANCE PLEASE ADD BALANCE FOR SEBL-BDT CURRENCY");
	         }else {
	        	 pw.println("BALANCE IS SUFFICIENT FOR SEBL-BDT CURRENCY");
	         }
	        // Thread.sleep(2000);
	         pw.println("*****Test Case 023 Passed For SEBL_SOUTHEAST_BANK_LTD*****");
	         pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	         pw.close();
	         }}
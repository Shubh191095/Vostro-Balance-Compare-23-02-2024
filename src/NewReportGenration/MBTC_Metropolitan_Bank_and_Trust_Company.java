...
public class MBTC_Metropolitan_Bank_and_Trust_Company {
	public void testBase016(WebDriver driver) throws InterruptedException, IOException
	{
...
//=====================================================================================================================================		
			...
			driver.findElement(By.xpath("/html/body/div/div[1]/div/div/div/section/div/div/div/div[2]/div/div[1]/select/option[27]")).click();
			Thread.sleep(2000);
			   ...
			   //Identify the pop-up element using XPath (adjust the XPath as needed)
			     ...
			     okButton.click();
			       ...
			       //Take Screen Shots for Select the Partner
				    ...
				    Files.copy(f6, new File("C:\\Users\\Shubham\\Desktop\\New Automation Project Related Docs\\Automation Screenshots\\Vostro Balance For MBTC_Metropolitan_Bank_and_Trust_Company.jpg"));
...
//==========================================================================================================================================================         
	         ...
	         pw.println("Threshold Balance For MBTC-PHP: "+properties.getProperty("MBTC-PHP"));
	         pw.println("Threshold Balance For MBTC-USD: "+properties.getProperty("MBTC-USD"));
	  
	        //For PHP Currency
	 		if(Double.valueOf(vostroBalancePHP)  < Double.valueOf(properties.getProperty("BPI-PHP"))) {

	 			 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR MBTC-PHP CURRENCY");
	 		}else if(Double.valueOf(properties.getProperty("MBTC-PHP")) <  Double.valueOf(vostroBalancePHP) && Double.valueOf(vostroBalancePHP)  < Double.valueOf(properties.getProperty("MBTC_Near_To_Threshold_Balance"))) {
	 			 pw.println("REACHING NEAR TO THRESHOLD BALANCE FOR MBTC-PHP PLEASE ADD BALANCE");
	 		}else {
	 			 pw.println("BALANCE IS SUFFICIENT FOR MBTC-PHP CURRENCY");
	 		}
	 		Thread.sleep(2000);
	        ...
	        //For USD Currency		
	 		if(Double.valueOf(vostroBalanceUSD)  < Double.valueOf(properties.getProperty("BPI-PHP"))) {
	 			 ...
	 			 pw.println("BALANCE IS BELOW THRESHOLD BALANCE FOR MBTC-USD CURRENCY");
	 		}else if(Double.valueOf(properties.getProperty("MBTC-USD")) <  Double.valueOf(vostroBalanceUSD) && Double.valueOf(vostroBalanceUSD)  < Double.valueOf(properties.getProperty("MBTC_Near_To_Threshold_Balance"))) {
	 			 pw.println("REACHING NEAR TO THRESHOLD BALANCE FOR MBTC-USD PLEASE ADD BALANCE");
	 		}else {
	 			 pw.println("BALANCE IS SUFFICIENT FOR FOR MBTC-USD CURRENCY");
	 		}
	 		//Thread.sleep(2000);	
	 	     pw.println("*****Test 016 Case Passed For MBTC_Metropolitan_Bank_and_Trust_Company*****");
	 		 pw.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	 		 pw.close();
	 		}}